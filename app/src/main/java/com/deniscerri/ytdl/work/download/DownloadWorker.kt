...
//                            }
    ...
    private suspend fun resolveMusicTags(item: DownloadItem, dataUpdate: Job): MusicMetadata? {
        ...
        val card = item.audioPreferences.musicMetadata?.takeIf { it.isUsable }

        val song = runCatching {
            when {
                //the card resolved the song, but a download started early can leave with one
                //whose completion was still on its way, so it is finished off here
                card != null -> if (card.lyrics.isBlank()) MusicMetadataUtil.complete(card) else card
                    ...
                    MusicMetadataUtil.resolveForVideo(item.title, item.author)
                }
            }
        }.getOrNull() ?: card ?: return null

        item.audioPreferences.musicMetadata = song
        return song
    }
